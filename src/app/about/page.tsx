import type { CSSProperties } from "react";

import { ContactSection } from "@/components/contact-section";
import { LazyImage } from "@/components/effects/lazy-image";
import { SiteFooter } from "@/components/site-footer";
import { SiteHeader } from "@/components/site-header";
import { aboutData } from "@/lib/ulrych-data";

function AboutHeading({
    children,
    size = "large",
}: {
    children: string;
    size?: "large" | "medium";
}) {
    return (
        <h2
            className={
                size === "large"
                    ? "max-w-[20ch] font-heading text-[28px] font-light leading-[1.05] tracking-tighter md:text-[48px]"
                    : "max-w-[20ch] font-heading text-[26px] font-light leading-[1.05] tracking-tighter md:text-[40px]"
            }
        >
            {children}
        </h2>
    );
}

function TimelineList({
    items,
}: {
    items: {
        role?: string;
        company?: string;
        degree?: string;
        institution?: string;
        period: string;
    }[];
}) {
    return (
        <ul className="mt-8 border-t border-border md:mt-12">
            {items.map((item) => (
                <li
                    className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 border-b border-border py-4 last:border-b-0 md:py-5"
                    key={`${item.role ?? item.degree}-${item.company ?? item.institution}-${item.period}`}
                >
                    <div className="flex min-w-0 flex-wrap items-baseline gap-x-3 gap-y-0.5">
                        <span className="text-[15px] leading-[1.5] text-foreground md:text-[16px]">
                            {item.role ?? item.degree}
                        </span>
                        <span className="text-[14px] leading-[1.5] text-muted-foreground md:text-[15px]">
                            {item.company ?? item.institution}
                        </span>
                    </div>
                    <span className="shrink-0 text-[13px] leading-[1.5] text-muted-foreground md:text-[14px]">
                        {item.period}
                    </span>
                </li>
            ))}
        </ul>
    );
}

function TagList({ items }: { items: string[] }) {
    return (
        <ul className="mt-8 flex flex-wrap gap-2 md:mt-12">
            {items.map((item) => (
                <li
                    className="inline-flex h-9 items-center rounded-full border border-border px-4 text-[14px] leading-none text-foreground"
                    key={item}
                >
                    {item}
                </li>
            ))}
        </ul>
    );
}

export default function AboutPage() {
    return (
        <>
            <SiteHeader />
            <main className="relative z-10">
                <section className="relative px-5 pb-16 pt-32 md:px-10 md:pb-24 md:pt-[8.75rem]">
                    <div className="mx-auto max-w-[89.5rem]">
                        <div data-reveal>
                            <h1 className="max-w-[22ch] font-heading text-[36px] font-light leading-[0.95] tracking-tighter text-foreground md:text-[clamp(56px,5.4vw,80px)]">
                                {aboutData.headline}
                            </h1>
                            <div className="mt-10 grid max-w-195 gap-6 text-[17px] leading-[1.55] text-foreground/85 md:mt-14 md:max-w-250 md:text-[18px]">
                                <p>{aboutData.subhead}</p>
                                <p>
                                    I enjoy finding the balance between a clean
                                    interface, a distinctive visual direction,
                                    and the technical reality of a project. I
                                    care not only about how a design looks at
                                    first glance, but also whether it makes
                                    sense for users, clients, and development
                                    teams.
                                </p>
                            </div>
                        </div>

                        <div
                            className="relative mt-16 aspect-[16/9] w-full overflow-hidden bg-white/5 md:mt-20"
                            data-reveal
                            style={
                                { "--reveal-delay": "120ms" } as CSSProperties
                            }
                        >
                            <LazyImage
                                alt="Kristian working at a desk"
                                className="object-cover"
                                fill
                                sizes="(min-width: 768px) 94vw, 100vw"
                                src="/images/ulrychkristian/about_desk.jpg"
                            />
                        </div>
                    </div>
                </section>

                <section className="relative px-5 py-16 md:px-10 md:py-24">
                    <div className="mx-auto grid max-w-[89.5rem] gap-12 md:grid-cols-2 md:gap-20">
                        <div data-reveal>
                            <AboutHeading>
                                How I think about design
                            </AboutHeading>
                            <div className="mt-8 flex flex-col gap-6 text-[17px] leading-[1.55] text-foreground/85 md:mt-12 md:max-w-200">
                                {aboutData.designPhilosophy.map((paragraph) => (
                                    <p key={paragraph}>{paragraph}</p>
                                ))}
                            </div>
                        </div>

                        <div
                            className="relative min-h-[439px] overflow-hidden bg-white/5 md:min-h-0"
                            data-reveal
                            style={
                                { "--reveal-delay": "120ms" } as CSSProperties
                            }
                        >
                            <LazyImage
                                alt="Kristian at work"
                                className="object-cover"
                                fill
                                sizes="(min-width: 768px) 40vw, 100vw"
                                src="/images/ulrychkristian/about_working.jpg"
                            />
                        </div>
                    </div>
                </section>

                <section className="relative px-5 py-16 md:px-10 md:py-24">
                    <div className="mx-auto grid max-w-[89.5rem] gap-x-12 gap-y-14 md:grid-cols-2">
                        <div data-reveal>
                            <AboutHeading size="medium">
                                Work experience
                            </AboutHeading>
                            <TimelineList items={aboutData.experiences} />
                        </div>
                        <div
                            data-reveal
                            style={
                                { "--reveal-delay": "90ms" } as CSSProperties
                            }
                        >
                            <AboutHeading size="medium">Education</AboutHeading>
                            <TimelineList items={aboutData.education} />
                        </div>
                    </div>
                </section>

                <section className="relative px-5 py-16 md:px-10 md:py-24">
                    <div className="mx-auto grid max-w-[89.5rem] gap-x-12 gap-y-14 md:grid-cols-2">
                        <div data-reveal>
                            <AboutHeading size="medium">Skills</AboutHeading>
                            <TagList items={aboutData.skills} />
                        </div>
                        <div
                            data-reveal
                            style={
                                { "--reveal-delay": "90ms" } as CSSProperties
                            }
                        >
                            <AboutHeading size="medium">Tools</AboutHeading>
                            <TagList items={aboutData.tools} />
                        </div>
                    </div>
                </section>

                <section className="relative px-5 py-16 md:px-10 md:py-24">
                    <div className="mx-auto max-w-[89.5rem]">
                        <div
                            className="grid gap-x-12 gap-y-8 md:grid-cols-12"
                            data-reveal
                        >
                            <div className="md:col-span-4">
                                <h2 className="max-w-[14ch] font-heading text-[28px] font-light leading-[1.05] tracking-tighter md:text-[48px]">
                                    Beyond screens
                                </h2>
                            </div>
                            <div className="md:col-span-8">
                                <div className="max-w-[52rem] space-y-5 text-[17px] leading-[1.55] text-foreground/85 md:text-[19px]">
                                    {aboutData.beyondScreens.map((paragraph) => (
                                        <p key={paragraph}>{paragraph}</p>
                                    ))}
                                </div>
                            </div>
                        </div>

                        <div className="mt-14 md:mt-20">
                            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-6">
                                {aboutData.beyondScreensImages.map(
                                    (image, index) => (
                                        <div
                                            className="relative aspect-[4/5] w-full overflow-hidden bg-white/5"
                                            data-reveal
                                            key={image.src}
                                            style={
                                                {
                                                    "--reveal-delay": `${Math.min(index, 3) * 70}ms`,
                                                } as CSSProperties
                                            }
                                        >
                                            <LazyImage
                                                alt={image.alt}
                                                className="object-cover"
                                                fill
                                                sizes="(min-width: 768px) 22vw, 50vw"
                                                src={image.src}
                                            />
                                        </div>
                                    ),
                                )}
                            </div>
                        </div>
                    </div>
                </section>

                <section className="relative px-5 py-16 md:px-10 md:py-24">
                    <div className="mx-auto max-w-[89.5rem]">
                        <div data-reveal>
                            <AboutHeading>Albums on repeat</AboutHeading>
                        </div>
                        <div className="mt-8 grid grid-cols-2 gap-3 md:mt-12 md:grid-cols-6 md:gap-6">
                            {aboutData.albums.map((album, index) => (
                                <div
                                    className="relative aspect-square overflow-hidden bg-white/5"
                                    data-reveal
                                    key={album.src}
                                    style={
                                        {
                                            "--reveal-delay": `${Math.min(index, 5) * 45}ms`,
                                        } as CSSProperties
                                    }
                                >
                                    <LazyImage
                                        alt={album.alt}
                                        className="object-cover"
                                        fill
                                        sizes="(min-width: 768px) 15vw, 50vw"
                                        src={album.src}
                                    />
                                </div>
                            ))}
                        </div>
                    </div>
                </section>

                <ContactSection />
            </main>
            <div className="relative z-10">
                <SiteFooter />
            </div>
        </>
    );
}
