...
export function TestimonialSection({ items, title }: TestimonialSectionProps) {
  ...
  const prev = () =>
    changeTestimonial((currentIndex - 1 + items.length) % items.length);
  ...
  return (
    <section className="px-5 py-16 md:px-10 md:py-32" data-reveal>
      ...
      <div className="mt-14 border-y border-border py-12 md:py-20 relative min-h-75">
        <article
        ...
        >
          ...
          <div className="text-[14px] leading-normal md:col-span-4 md:text-right">
            <p>{featured.author}</p>
            <p className="mt-1 text-muted-foreground">{featured.role}</p>
          </div>
        </article>
